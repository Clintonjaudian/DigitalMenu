        ...
        <div class="row">
            ...
            <div class="card card-info">
                ...
                <div class="float-right">
                    <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#addLowerLeftProductModalBtn">Add Product</button>
                </div>

                <table id="lowerLeftDatatable" class="table table-bordered table-striped" width="100%">
                    <thead>
                    <tr>
                    <th>Product Name</th>
...
<div class="modal" tabindex="-1" role="dialog" id="addLowerLeftProductModalBtn">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card card-info">
              ...
              <div class="card-body">
                <form method="POST" id="addLowerLeftForm" enctype="multipart/form-data"> 
                  @csrf
                  ...
                  <div class="form-group float-right">
                    <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal" id="addLowerLeftCancelBtn">Cancel</button>
...
<div class="modal" tabindex="-1" role="dialog" id="editLowerLeftProductModalBtn">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card card-info">
              ...
              <div class="card-body">
                <form method="POST" id="editLowerLeftForm" enctype="multipart/form-data"> 
                  @csrf
                  @method('PUT')
...
@push('js')
    ...
    <script>
        ...
        {
          // Yajra DataTables
          $("#lowerLeftDatatable").DataTable(
            {
              processing:true,
              serverSide:true,
              ajax:{
                url:"{{route('lower_left_menu1.index')}}"
              },
              columns:[
                {
                  data:'product_name',
          ...
          // Parsley Validator
          $("#addLowerLeftForm").parsley();

          // Add Form Cancel Btn
          $("#addLowerLeftCancelBtn").on('click', function()
          {
            $("#addLowerLeftForm").parsley().reset();
          });

          // Add Lower Left Product
          $("#addLowerLeftForm").on('submit', function(event)
          {
            ...
            $.ajax(
              {
                url:"{{route('lower_left_menu1.store')}}",
                method:'post',
                data:formData,
                dataType:'json',
                cache:false,
                ...
                success:function(data)
                {
                  ...
                  else
                  {
                    if(data.validator)
                    {
                      $("#addLowerLeftProductModalBtn").modal('hide');
                      $("#lowerLeftDatatable").DataTable().ajax.reload();
                      $("#addLowerLeftForm")[0].reset();
                      $("#addLowerLeftForm").parsley().reset();
            ...
            $.ajax(
              {
                ...
                success:function(data)
                {
                  ...
                  $("#editLowerLeftProductModalBtn").modal('show');

                  $("#editLowerLeftForm").unbind('submit').bind('submit', function(event)
                  {
                    event.preventDefault();
                    var formData = new FormData($(this)[0]);
                    ...
                    $.ajax(
                      {
                        ...
                        success:function(data)
                        {
                          if(data.validator)
                          {
                            $("#editLowerLeftProductModalBtn").modal('hide');
                            $("#lowerLeftDatatable").DataTable().ajax.reload();
                            $("#editLowerLeftForm")[0].reset();
                            $("#editLowerLeftForm").parsley().reset();
